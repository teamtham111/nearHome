...
WORKER_URL="${ENRICHMENT_WORKER_URL:-}"
TASK_SERVICE_ACCOUNT="${CLOUD_TASKS_SERVICE_ACCOUNT_EMAIL:-}"
TASK_AUDIENCE="${CLOUD_TASKS_OIDC_AUDIENCE:-$WORKER_URL}"
...
: "${WORKER_URL:?Set ENRICHMENT_WORKER_URL to the private worker HTTPS URL}"
...
EGRESS_DIAGNOSTICS_TOKEN_SECRET_NAME="${EGRESS_DIAGNOSTICS_TOKEN_SECRET_NAME:-nearhome-egress-diagnostics-token}"
...
if gcloud secrets describe "$EGRESS_DIAGNOSTICS_TOKEN_SECRET_NAME" >/dev/null 2>&1; then
  gcloud secrets add-iam-policy-binding "$EGRESS_DIAGNOSTICS_TOKEN_SECRET_NAME" \
    --member="serviceAccount:$SERVICE_ACCOUNT" --role="roles/secretmanager.secretAccessor" --quiet >/dev/null
  SECRET_MAPPINGS+=",EGRESS_DIAGNOSTICS_TOKEN=$EGRESS_DIAGNOSTICS_TOKEN_SECRET_NAME:latest"
fi
