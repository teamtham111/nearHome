...
echo "  ARQ worker (optional): cd apps/api && source .venv/bin/activate && JOB_EXECUTION_MODE=arq REDIS_URL=redis://localhost:6379/0 python -m app.jobs.worker"
echo ""
echo "Or run: docker compose up --build (add --profile arq for the optional worker)"
